...
const formatTokens: {[key: string]: {type: string, func: Function}} = {
  h: {
    ...
    func: (milliSecond: number, options: any = {}) => {
      ...
      const value = decimal(hour, options.decimalPlace);
  ...
  m: {
    ...
    func: (milliSecond: number, options: any = {}) => {
      ...
      const value = decimal(minute, options.decimalPlace);
  ...
  s: {
    ...
    func: (milliSecond: number, options: any = {}) => {
      ...
      const value = decimal(second, options.decimalPlace);
  ...
  S: {
    type: 'millisecond',
    func: (milliSecond: number, options: any = {}) => {
      const value = decimal(milliSecond, options.decimalPlace);
      return [value, 0];
    },
  },
};
